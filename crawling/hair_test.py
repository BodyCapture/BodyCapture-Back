...
def add_file(key_word):
    ...
    csv_filename = f'{key_word} 헤어샵 임시.csv'
    df.to_csv(csv_filename, encoding='euc-kr', index=False)
    print(f'============={key_word} 헤어샵 csv파일 완성=============')
    ...
    final_csv_filename = f'{key_word} 헤어샵.csv'
    df.to_csv(final_csv_filename, encoding='euc-kr')
    print(f'============={key_word} 헤어샵 csv파일 수정 완료!!=============')
...
def search_data(key_word) :
    ...
    # 검색
    search.send_keys(f'{key_word} 헤어샵')  
    ...
    store_list = driver.find_elements(By.CSS_SELECTOR, 'li.p0FrU')  # 헤어샵일 때 가게리스트
    # 페이지 리스트
    ...
    for btn in range(len(next_btn))[1:] :  
        ...
        names = driver.find_elements(By.CSS_SELECTOR, 'span.O_Uah')  # 헤어샵일 때 상호명
        ...
        link_buttons = driver.find_elements(By.CSS_SELECTOR, '.QTjRp > a')  # 헤어샵일 때 상세정보 링크
        ...
        for data in range(len(store_list)):  
            ...
            try:   
                ...
                address_element = time_wait(20, 'span.LDgIH')
